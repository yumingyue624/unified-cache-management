...
public:
                 ...
                 QueryResult& result) override
    {
        ...
        for (const auto& key : keys) { result.exists.emplace_back(key == MakeCacheKey("k15") || key == MakeCacheKey("k25")); }
...
CacheKey FindKeyForAsu(const std::vector<AsuId>& asuIds, AsuId targetAsuId)
{
    ...
    for (std::size_t index = 0; index < 10000; ++index) {
        auto key = MakeCacheKey("route-key-" + std::to_string(targetAsuId) + "-" +
                                std::to_string(index));
...
TEST(AsuClientImplTest, Query_PerKeyKeepsOriginalOrder)
{
    ...
    QueryResult result;
    auto status = client->Query({MakeCacheKey("k05"), MakeCacheKey("k15"), MakeCacheKey("k25")}, QueryOptions{}, result);
...
TEST(AsuClientImplTest, Query_PrefixBroadcastsAndMergesResults)
{
    ...
    QueryResult result;
    auto status = client->Query({MakeCacheKey("prefix-a"), MakeCacheKey("prefix-b"), MakeCacheKey("prefix-c")}, options, result);
...
TEST(AsuClientImplTest, Query_PrefixResultSizeMismatchReturnsInternalError)
{
    ...
    options.mode = QueryMode::PREFIX;
    QueryResult result;
    auto status = client->Query({MakeCacheKey("prefix-a"), MakeCacheKey("prefix-b")}, options, result);
