...
UC::Status ToUcStatus(const UC::ASU::Status& status, const char* operation)
{
    if (status.ok()) { return UC::Status::OK(); }

    const auto message = std::string{operation} + ": " + status.message;
    switch (status.code) {
        case UC::ASU::StatusCode::INVALID_ARGUMENT: return UC::Status::InvalidParam(message);
        case UC::ASU::StatusCode::NOT_FOUND: return UC::Status::NotFound();
        case UC::ASU::StatusCode::RESOURCE_BUSY: return UC::Status::NoSpace();
        case UC::ASU::StatusCode::TIMEOUT: return UC::Status::Timeout();
        default: return UC::Status::Error(message);
    }
}
...
UC::Expected<BufferSlot> AllocateBuffer(DramPoolRuntime& runtime, std::uint32_t len)
{
    if (runtime.bufferManagers.size() != g_config.poolBlockSizes.size()) {
        return UC::Status::Error("buffer manager layout does not match g_config");
    }
    ...
    return foundSuitablePool ? UC::Status::NoSpace()
                             : UC::Status::Error("no buffer pool fits len=" +
                                                 std::to_string(len));
}
