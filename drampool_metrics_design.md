...
每个 host 运行一个独立的 DramPool 进程，同一 host 上可能有多个 DP Scheduler/EngineCore 进程连接它。DramPool 不在 vLLM 进程内，因此不能直接把进程内指标暴露给 vLLM 的 metrics 系统。

本方案采用与 YuanRong 相同的跨进程回流模式：DramPool 将指标周期性写入本地文件，UCM Scheduler 中的后台 Reporter 读取最新快照，再把数据写入 Scheduler 进程内的 `ucmmetrics`，最终复用现有 `vllm_connector` 出口。

> **设计结论**：跨进程链路参考 YuanRong；DramPool 进程内采集选择 UCM 已有的 **thread-local + 双 buffer** 实现，不重新实现 YuanRong 的全局 `MetricSlot` 体系。

完整架构图见：[DramPool Metrics 跨进程架构图](./drampool_metrics_architecture.excalidraw)。该文件可以直接导入 Excalidraw 查看和编辑。
...
候选方案包括 YuanRong 的固定 `MetricSlot` 和 UCM 的 thread-local 双 buffer。两种实现的结构对比见：[进程内采集方案对比图](./drampool_metrics_buffer_comparison.excalidraw)。
...
| Gauge | `store()`、`fetch_add()` 或 `fetch_sub()` | 读取当前值，与上一次值比较 |
...
| 对比项 | YuanRong MetricSlot | UCM thread-local + 双 buffer |
| --- | --- | --- |
| 线程写入位置 | 多线程共享固定 slot | 每个线程写自己的 buffer |
| Counter/Gauge | 原子变量 | thread-local map 普通更新 |
| Histogram | 共享 slot，需要 `histMutex` | 每线程独立 bucket，无跨线程竞争 |
| 周期读取 | 扫描 slot，使用 `g_last` 算 delta | 原子切换 buffer，聚合旧 buffer |
| DramPool 接入成本 | 需要移植一套实现 | UCM 仓已有代码、测试和指标出口 |
| 最终选择 | 不采用 | **采用** |

最终复用 UCM 的 `MetricBuffer` 和聚合逻辑。DramPool 不引入 `g_slots` 或新的全局服务定位器；`DramPoolMetrics` 和 `DramPoolMetricsReporter` 由 `DramPoolServer` 持有，Worker、Poller 等运行组件通过 `DramPoolRuntime&` 使用它们。
...
Metrics 文件使用已有 `g_config.logDir`：

```text
${g_config.logDir}/drampool_metrics.log
```

采集周期固定为 10 秒。本方案不增加 `drampool.md` 中未定义的命令行参数。
...
- 不需要租约或心跳。
- 抢锁失败的 Reporter 本轮不采集，但线程不退出；下一轮继续尝试。

最后一点与当前 YuanRong Reporter 不同。YuanRong 当前实现抢锁失败后直接退出，DramPool 为了支持自动接任，需要改为循环重试。
...
处理顺序选择“先回流、后更新 state”。这样不会因为提前更新 state 而永久丢失指标；如果进程恰好在两步之间崩溃，接任者可能重复回流上一轮数据。该链路不引入事务协调，语义为偏向不丢数据的 at-least-once。
...
| Leader 退出 | fd 关闭，其他 Scheduler 下一轮接任 |
| DramPool Counter reset | `delta=current`，不产生负数 |
| 文件写入失败 | 记录错误，DramPool 业务继续运行 |

## 10. 测试方案

测试辅助代码、fake 和 fixture 均放在 `test/` 目录。

### 10.1 C++ 单元测试

- 多业务线程持续打点，同时反复切换 buffer，Counter 总数不丢失。
- 切换发生在 `BeginWrite()` 竞争窗口时，旧 buffer 不再被继续修改。
- 多线程 Histogram bucket 和 sum 聚合正确。
- Reporter 汇总周期增量后能够生成正确的累计快照。
- `ScopedTimer` 只产生一次 Histogram observation。
- metrics 文件写入失败不影响 DramPool 请求处理。

### 10.2 Python 单元测试

- 从小于和大于 64 KiB 的文件尾部读取最新记录。
- 忽略未写完的尾部半行。
- Counter、Histogram delta 和 reset 逻辑正确。
- State 使用临时文件加 `os.replace()` 原子更新。
- 多个 Reporter 中只有一个持有 leader 锁。
- Leader 退出后 loser 在下一轮成功接任。
- `ucmmetrics.update_stats()` 收到正确的 Counter/Gauge 增量。

### 10.3 验收标准

1. DramPool 热路径不进行文件 IO。
2. 10 秒快照连续生成，每一行都可独立解析。
3. 同一 DramPool 的节点指标不会被多个 Scheduler 重复回流。
4. Leader 崩溃后，无需人工干预即可自动接任。
5. 指标最终能够从 vLLM Prometheus 端点查询。
6. 关闭 metrics 或 metrics 链路异常时，DramPool 的 DUMP、LOAD、LOOKUP 功能保持正常。
